...
func (s *BlockChainAPI) CallEx(ctx context.Context, args TransactionArgs, blockNrOrHash rpc.BlockNumberOrHash, overrides *StateOverride, debug bool, needLogs bool) (*ExecutionResultEx, error) {
...
func (s *TransactionAPI) AllPendingTransactions() ([]*RPCTransaction, error) {
	pending, err := s.b.GetPoolTransactions()
	if err != nil {
		return nil, err
	}
	...
	transactions := make([]*RPCTransaction, 0, len(pending))
	for _, tx := range pending {
		transactions = append(transactions, newRPCPendingTransaction(tx, s.b.CurrentHeader(), s.b.ChainConfig()))
	}
