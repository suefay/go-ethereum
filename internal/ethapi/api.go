...
func DoCallEx(ctx context.Context, b Backend, args TransactionArgs, blockNrOrHash rpc.BlockNumberOrHash, overrides *StateOverride, timeout time.Duration, globalGasCap uint64, debug bool, needLogs bool) (*core.ExecutionResult, []byte, []byte, error) {
	...
	evm, vmError, err := b.GetEVM(ctx, msg, state, header, &vm.Config{Tracer: debugLogger, NoBaseFee: true})
	if err != nil {
		return nil, nil, nil, err
	}
	// Wait for the context to be done and cancel the evm. Even if the
	...
	if err != nil {
		return result, nil, nil, fmt.Errorf("err: %w (supplied gas %d)", err, msg.GasLimit)
	}
...
func (s *BlockChainAPI) CallEx(ctx context.Context, args TransactionArgs, blockNrOrHash rpc.BlockNumberOrHash, overrides *StateOverride, debug bool, needLogs bool) (hexutil.Bytes, error) {
