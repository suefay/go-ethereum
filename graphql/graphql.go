...
func (b *Block) Call(ctx context.Context, args struct {
	...
	result, err := ethapi.DoCall(ctx, b.backend, args.Data, *b.numberOrHash, nil, b.backend.RPCEVMTimeout(), b.backend.RPCGasCap(), false, false)
	if err != nil {
		return nil, err
	}
	status := Long(1)
...
func (p *Pending) Call(ctx context.Context, args struct {
	...
	pendingBlockNr := rpc.BlockNumberOrHashWithNumber(rpc.PendingBlockNumber)
	result, err := ethapi.DoCall(ctx, p.backend, args.Data, pendingBlockNr, nil, p.backend.RPCEVMTimeout(), p.backend.RPCGasCap(), false, false)
	if err != nil {
		return nil, err
	}
	status := Long(1)
